...
<Container {id} overrideDefaultStyling={true} class={["inline-flex w-full", variant].join(" ")}>
  <AnchoredOverlay
    ...
    {#snippet anchor({ toggle })}
      ...
      >
          ...
          {#if selectedItem !== undefined}
            <Container
              ...
              class={[
                ...
                selectedItem.avatarImage ? "overflow-hidden p-0" : getSearchableSelectorAvatarBackground(selectedItem.label),
              ].join(" ")}
            >
              {#if selectedItem.avatarImage}
                <Image
                  id={`${id}-trigger-avatar-image`}
            ...
            <Text id={`${id}-trigger-label`} type="span" class="truncate text-sm font-medium text-neutral-900 dark:text-neutral-100"
              >{selectedItem.label}</Text
            >
          {:else}
            <Container
              id={`${id}-trigger-avatar-placeholder`}
              overrideDefaultStyling={true}
              class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-neutral-100 text-xs font-medium text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400"
            >
        ...
        <Container
          id={`${id}-trigger-arrow`}
          overrideDefaultStyling={true}
          class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-neutral-400"
        >
        ...
        {#each filteredItems as item (item.value)}
          ...
          >
            <Container
              ...
              class={[
                "flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-semibold text-white",
                item.avatarImage ? "overflow-hidden p-0" : getSearchableSelectorAvatarBackground(item.label),
              ].join(" ")}
            >
              {#if item.avatarImage}
                <Image
                  id={`${id}-option-${item.value}-avatar-image`}
            ...
            <Text id={`${id}-option-${item.value}-label`} type="span" class="flex-1 truncate text-sm text-neutral-800 dark:text-neutral-200"
              >{item.label}</Text
            >
            {#if selectedValue === item.value}
              <Container
                id={`${id}-option-${item.value}-check`}
                overrideDefaultStyling={true}
                class="flex h-5 w-5 shrink-0 items-center justify-center text-primary-600"
              >
