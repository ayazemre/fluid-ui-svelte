...
<Container id={componentId} class={[variant, "fluid-popover-container", "inline-block"].join(" ")} onkeydown={handleKeydown}>
  <Button
    onclick={async () => toggle()}
    class={[variant, "fluid-popover-trigger", "inline-block"].join(" ")}
  ...
  {#if isOpen}
    <div
      ...
      role="presentation"
      class={[variant, "fluid-popover-content", `fluid-popover-${position}`].join(" ")}
