...
<Container id={componentId} class={mergeClasses(variant, "fluid-code-block-container flex flex-col overflow-scroll")}>
  {#each code.split("\n") as line, index}
    <Container class={mergeClasses(variant, "fluid-code-block-row flex items-start")}>
      {#if showLineNumbers}
        <Container class={mergeClasses(variant, "fluid-code-block-index sticky left-0 z-1 min-w-10 px-2")}>
