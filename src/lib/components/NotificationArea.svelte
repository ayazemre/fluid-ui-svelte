...
<Container class={[variant, "fluid-notification-area", `fluid-notification-area-${position}`].join(" ")}>
  {#each items as item (item.id)}
    <div
      animate:flip={{ duration: 300 }}
      in:fly={flyParams[position]}
      out:fade={{ duration: 200 }}
      class={[variant, "fluid-notification", `fluid-notification-${item.type || "info"}`].join(" ")}
    >
