...
<Container id={componentId} class={mergeClasses(variant, "fluid-calendar-wrapper flex flex-col")}>
  ...
  <Container class={mergeClasses(variant, "fluid-calendar-body grid grid-cols-7 justify-items-center")}>
    ...
    {#each calendarDays as cellDay}
      <Button
        ...
        class={mergeClasses(variant, generateCalendarCellStyles(currentDate, cellDay, startDate, endDate, hideRollingDays))}
      >
