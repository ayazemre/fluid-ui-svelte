 ...
 */
export function scrollToIndex(componentId: string, activeIndex: number, orientation: "horizontal" | "vertical" = "horizontal"): void {
  const element = document.getElementById(componentId);
  if (!element || !element.children[activeIndex]) return;

  const targetChild = element.children[activeIndex] as HTMLElement;
  const targetPosition = orientation === "horizontal" ? targetChild.offsetLeft : targetChild.offsetTop;
 ...
 */
export function calculateIndexFromScroll(element: HTMLElement, orientation: "horizontal" | "vertical" = "horizontal"): number {
...
export function handleScrollEnd(event: Event, orientation: "horizontal" | "vertical", activeIndex: number) {
