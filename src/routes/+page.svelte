...
>
  ...
  <Container class="z-10 flex flex-1 flex-col items-center justify-center gap-6 bg-transparent px-4 py-16 md:p-16">
    <Container class="flex text-center">
      <Text type="h1" class="text-4xl font-bold text-neutral-950 sm:text-5xl md:text-6xl dark:text-neutral-50"
        >Build with <Text type="span" class="bg-linear-to-r from-primary-600 to-secondary-300 bg-clip-text text-transparent">Fluid UI</Text></Text
      >
    </Container>
    <Container class="flex">
      <Text class="text-center text-lg text-neutral-950 opacity-70 transition-colors sm:text-xl dark:text-neutral-50 dark:opacity-80"
        >A modern Svelte component library for building beautiful, responsive interfaces.</Text
      >
    </Container>
    <Container class="flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-row sm:gap-4" overrideDefaultStyling>
      <Button
        onclick={async () => {
    ...
    <Text class="absolute bottom-4 text-sm text-neutral-400!">{"© Fluid UI " + new Date().getFullYear()}</Text>
  </Container>
</Page>
