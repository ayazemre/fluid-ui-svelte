<script lang="ts">
  ...
  import { DatePicker, CodeBlock } from "#src/lib/components/index.ts";
...
<Container class="flex flex-col gap-10">
  ...
  <Container class="flex flex-col gap-4">
    ...
    <Container class="rounded-xl border p-6 dark:border-neutral-800">
      <Container class="mb-4 flex gap-2">
        <Button class="fluid-button-secondary" onclick={async () => changeMonthMulti(-1)}>Prev</Button>
        <Button class="fluid-button-secondary" onclick={async () => changeMonthMulti(1)}>Next</Button>
      </Container>
        ...
        <Container class="max-w-sm rounded border p-4 dark:border-neutral-800">
          <DatePicker
            currentDate={new Date(
              new Date(multiCalendarState.currentDate).setMonth(new Date(multiCalendarState.currentDate).getMonth() + 1),
            ).toISOString()}
            bind:startDate={multiCalendarState.startDate}
            bind:endDate={multiCalendarState.endDate}
